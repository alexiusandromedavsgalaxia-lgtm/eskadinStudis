...
function Account(){const t=useTranslation();return <main className="page narrow"><div className="eyebrow">{t.account}</div><h1 className="page-title">Player profile.</h1><div className="form-card"><div className="account-avatar">ES</div><h2>Eskådin Player</h2><p className="muted">0 published games · 0 sessions · 0 F¢</p><div className="account-actions"><Link className="button button-ghost" to="/editor">{t.create}</Link><Link className="button button-ghost" to="/statistics">{t.stats}</Link><Link className="button danger-button" to="/account/delete">Delete account</Link></div><Link className="muted" to="/">{t.back}</Link></div></main>}
function Login(){const t=useTranslation();return <main className="page narrow"><div className="form-card"><div className="eyebrow">{t.account}</div><h1>{t.login}</h1><label>Email<input type="email"/></label><label>Password<input type="password"/></label><button className="button button-primary">{t.login}</button><Link className="muted" to="/register">{t.register}</Link></div></main>}
function Register(){const t=useTranslation();return <main className="page narrow"><div className="form-card"><div className="eyebrow">{t.account}</div><h1>{t.register}</h1><label>Username<input/></label><label>Email<input type="email"/></label><label>Password<input type="password"/></label><button className="button button-primary">{t.register}</button><Link className="muted" to="/login">{t.login}</Link></div></main>}
...
export default function App(){return <Layout><Routes><Route path="/" element={<Home/>}/><Route path="/games" element={<Games/>}/><Route path="/games/:id" element={<Game/>}/><Route path="/games/:id/play" element={<Play/>}/><Route path="/editor" element={<Editor/>}/><Route path="/developer" element={<Developer/>}/><Route path="/publish" element={<Publish/>}/><Route path="/statistics" element={<Statistics/>}/><Route path="/account" element={<Account/>}/><Route path="/login" element={<Login/>}/><Route path="/register" element={<Register/>}/><Route path="/account/delete" element={<DeleteAccount/>}/><Route path="*" element={<NotFound/>}/></Routes></Layout>}
