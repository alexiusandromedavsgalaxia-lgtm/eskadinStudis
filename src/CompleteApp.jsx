...
async function makeCloudKey(email,password){const value=String(email||"").trim().toLowerCase()+"\\n"+String(password||"");if(!value.trim())return null;const data=new TextEncoder().encode(value);const digest=await crypto.subtle.digest("SHA-256",data);return Array.from(new Uint8Array(digest)).map(b=>b.toString(16).padStart(2,"0")).join("")}
function CloudSync(){const{user}=useUser();useEffect(()=>{const key=user?.cloudKey;if(!key)return;let cancelled=false;const sync=async()=>{try{const res=await fetch("/api/projects",{cache:"no-store",headers:{"X-Eskadin-Key":key}});if(!res.ok)throw new Error("cloud projects "+res.status);const data=await res.json();const remote=Array.isArray(data.projects)?data.projects:[];const local=projectList();const byId=new Map();[...remote,...local].forEach(p=>{if(p?.id)byId.set(String(p.id),p)});const merged=[...byId.values()];if(cancelled)return;saveProjectListLocal(merged);if(!remote.length&&local.length){await fetch("/api/projects",{method:"PUT",headers:{"Content-Type":"application/json","X-Eskadin-Key":key},body:JSON.stringify({projects:local})})}else if(remote.length&&merged.length!==remote.length){await fetch("/api/projects",{method:"PUT",headers:{"Content-Type":"application/json","X-Eskadin-Key":key},body:JSON.stringify({projects:merged})})}}catch{} };sync();return()=>{cancelled=true}},[user?.cloudKey]);return null}
function AuthProvider({children}){const[user,setUser]=useState(()=>read("eskadin-user",null));const[developer,setDeveloper]=useState(()=>read("eskadin-developer",null));const login=async(u)=>{const next={...u};if(u.email&&u.password)next.cloudKey=await makeCloudKey(u.email,u.password);delete next.password;setUser(next);save("eskadin-user",next);return next};const logout=()=>{setUser(null);localStorage.removeItem("eskadin-user")};const devLogin=u=>{setDeveloper(u);save("eskadin-developer",u)};const devLogout=()=>{setDeveloper(null);localStorage.removeItem("eskadin-developer")};return <AuthContext.Provider value={{user,developer,login,logout,devLogin,devLogout}}><CloudSync/>{children}</AuthContext.Provider>}
function useGames(){const{user}=useUser();const cloudKey=user?.cloudKey||null;const[games,setGames]=useState(()=>{if(localStorage.getItem("eskadin-games-version")!==CATALOG_VERSION){localStorage.setItem("eskadin-games-version",CATALOG_VERSION);localStorage.removeItem("eskadin-games");return seed}return read("eskadin-games",seed)});const syncReady=useRef(false);useEffect(()=>{let cancelled=false;syncReady.current=false;const load=async()=>{if(!cloudKey){syncReady.current=true;return}try{const res=await fetch("/api/games",{cache:"no-store",headers:{"X-Eskadin-Key":cloudKey}});if(!res.ok)throw new Error("cloud get "+res.status);const data=await res.json();const remote=Array.isArray(data.games)?data.games:[];const local=read("eskadin-games",seed);const byId=new Map();[...remote,...local].forEach(g=>{if(g?.id!=null)byId.set(String(g.id),g)});const merged=[...byId.values()];if(cancelled)return;setGames(merged);save("eskadin-games",merged);if(!remote.length&&local.length){await fetch("/api/games",{method:"PUT",headers:{"Content-Type":"application/json","X-Eskadin-Key":cloudKey},body:JSON.stringify({games:local})})}else if(remote.length&&merged.length!==remote.length){await fetch("/api/games",{method:"PUT",headers:{"Content-Type":"application/json","X-Eskadin-Key":cloudKey},body:JSON.stringify({games:merged})})}}catch{setGames(read("eskadin-games",seed))}finally{if(!cancelled)syncReady.current=true}};load();return()=>{cancelled=true}},[cloudKey]);useEffect(()=>{save("eskadin-games",games);if(!cloudKey||!syncReady.current)return;const timer=setTimeout(()=>{fetch("/api/games",{method:"PUT",headers:{"Content-Type":"application/json","X-Eskadin-Key":cloudKey},body:JSON.stringify({games})}).catch(()=>{})},350);return()=>clearTimeout(timer)},[games,cloudKey]);return[games,setGames]}
...
function createEskadinR15Avatar(user={}){
 ...
 const tapered=(parent,name,topW,bottomW,topD,bottomD,h,material,segments=10)=>{
   const g=new THREE.Group();g.name=name;
   const geo=new THREE.CylinderGeometry(topW/2,bottomW/2,h,segments,1,false);
   geo.scale(1,1,Math.max(.1,topD/Math.max(.001,topW)));
   const m=new THREE.Mesh(geo,material.clone());m.castShadow=true;m.receiveShadow=true;g.add(m);parent.add(g);return g;
 };
 ...
 const lUL=bone("LeftUpperLeg",-.31*sx,-.38*sy,0,lower),lLL=bone("LeftLowerLeg",0,-.60*sy,0,lUL),lF=bone("LeftFoot",0,-.55*sy,-.17,lLL);
 const rUL=bone("RightUpperLeg",.31*sx,-.38*sy,0,lower),rLL=bone("RightLowerLeg",0,-.60*sy,0,rUL),rF=bone("RightFoot",0,-.55*sy,-.17,rLL);
 const lUA=bone("LeftUpperArm",-.57*sx,.32*sy,0,upper),lLA=bone("LeftLowerArm",0,-.53*sy,0,lUA),lH=bone("LeftHand",0,-.48*sy,0,lLA);
 const rUA=bone("RightUpperArm",.57*sx,.32*sy,0,upper),rLA=bone("RightLowerArm",0,-.53*sy,0,rUA),rH=bone("RightHand",0,-.48*sy,0,rLA);
 ...
 tapered(rUL,"RightUpperLegMesh",.38*sx,.34*sx,.38,.34,.70*sy,pants,12).position.y=.02*sy;
 tapered(rLL,"RightLowerLegMesh",.34*sx,.30*sx,.34,.30,.64*sy,pants,12).position.y=.02*sy;
 tapered(lF,"LeftFootMesh",.42*sx,.38*sx,.64,.58,.24*sy,shoe,12).position.z=-.15;
 tapered(rF,"RightFootMesh",.42*sx,.38*sx,.64,.58,.24*sy,shoe,12).position.z=-.15;
 tapered(lUA,"LeftUpperArmMesh",.40*sx,.34*sx,.40,.34,.60*sy,shirt,12).position.y=.01*sy;
 tapered(lLA,"LeftLowerArmMesh",.34*sx,.29*sx,.34,.29,.54*sy,skin,12).position.y=.01*sy;
 tapered(rUA,"RightUpperArmMesh",.40*sx,.34*sx,.40,.34,.60*sy,shirt,12).position.y=.01*sy;
 ...
 attach("RightShoulderAttachment",rUA,[0,.29*sy,0]);attach("LeftShoulderAttachment",lUA,[0,.29*sy,0]);attach("BodyFrontAttachment",upper,[0,.10*sy,.25]);attach("BodyBackAttachment",upper,[0,.10*sy,-.25]);attach("WaistCenterAttachment",lower,[0,-.22*sy,0]);
...
function ThreeViewport({scene,selected,setSelected,tool,grid,upd,beginHistory,endHistory,wireframe=false,showAxes=true,snap=false}){
...
function saveProjectListLocal(list){save(PROJECTS_KEY,list)}
function saveProjectList(list){saveProjectListLocal(list);const key=read("eskadin-user",null)?.cloudKey;if(key)fetch("/api/projects",{method:"PUT",headers:{"Content-Type":"application/json","X-Eskadin-Key":key},body:JSON.stringify({projects:list})}).catch(()=>{})}
...
const STUDIO_OBJECT_CATALOG=(()=>{
 const base=[
  ["cube","Cubo"],["wall","Pared"],["sphere","Esfera"],["cylinder","Cilindro"],["cone","Cono"],["torus","Toro"],["capsule","Cápsula"],["plane","Plano"],["floor","Suelo"],["fence","Valla"],["stairs","Escaleras"],
  ["light","Luz"],["sound","Sonido"],["spawn","Punto de aparición"],["camera","Cámara"],["text","Texto"],["arch","Arco"],["door","Puerta"],["window","Ventana"],["roof","Tejado"],["pillar","Columna"],["beam","Viga"],["ramp","Rampa"],["platform","Plataforma"],["sign","Cartel"],["tree","Árbol"],["rock","Roca"],["bush","Arbusto"],["lamp","Farola"],["bench","Banco"],["table","Mesa"],["chair","Silla"],["crate","Caja"],["barrel","Barril"],["fountain","Fuente"],["bridge","Puente"],["tower","Torre"],["castle","Castillo"],["house","Casa"],["road","Carretera"],["path","Camino"],["water","Agua"],["lava","Lava"],["ice","Hielo"],["grass","Hierba"],["sand","Arena"],["brick","Ladrillo"],["metal","Metal"],["glass","Cristal"],["pipe","Tubería"],["gate","Puerta grande"],["signpost","Señal"],["checkpoint","Checkpoint"],["teleporter","Teletransportador"],["trigger","Zona de evento"],["spawner","Generador"],["button","Botón"],["lever","Palanca"],["switch","Interruptor"],["checkpoint","Punto de control"],["goal","Meta"],["coin","Moneda"],["gem","Gema"],["heart","Corazón"],["star","Estrella"],["platform","Plataforma móvil"],["vehicle","Vehículo"],["robot","Robot"],["npc","NPC"],["dummy","Maniquí"],["tree","Pino"],["palm","Palmera"],["flower","Flor"],["cactus","Cactus"],["cloud","Nube"],["sun","Sol"],["moon","Luna"]
 ];
 const out=[];for(let i=0;i<500;i++){const [type,label]=base[i%base.length];const family=Math.floor(i/base.length)+1;out.push([type,label+" "+family])}return out;
})();
 ...
 <div className="studio-project-manager-actions">
  ...
  <button title="Renombrar juego" onClick={()=>{const name=window.prompt("Nombre del juego",currentName);if(name?.trim()){setCurrentName(name.trim());}}}>✎ Renombrar</button>
    ...
    <div className="studio-sidebar-actions">
     <button title={t.add} onClick={()=>setPanel("create")}>＋ {t.objects}</button><button title={t.hierarchy} onClick={()=>setPanel("scene")}>☷ {t.hierarchy}</button><button title="Programación" onClick={()=>setPanel("scripts")}>⌘ Programación</button><Link title="Object Studio" to="/object-studio">◈ Object Studio</Link><button title={t.scene} onClick={()=>setPanel("scene")}>◉ {t.scene}</button>
    </div>
...
panel==="create"&&<><strong>{t.objects} · 500</strong><div className="studio-object-grid">{STUDIO_OBJECT_CATALOG.map(([type,label],i)=><button key={type+"-"+i} title={label} aria-label={label} onClick={()=>add(type)}>{label}</button>)}</div></>}{panel==="scene"&&<><strong>{t.hierarchy}</strong><div className="studio-scene-list">{scene.map(o=><button key={o.id} title={o.name} className={selected===o.id?"active":""} onClick={()=>setSelected(o.id)}>{o.name}</button>)}</div></>}</div><div className="studio-ai-panel"><div><b>✦ Eskådin IA</b><span>entiende y edita el entorno actual</span></div><div className="studio-ai-context">{aiMessage}</div><textarea value={aiPrompt} onChange={e=>setAiPrompt(e.target.value)} placeholder="ej. cambia las casas a rojo y añade árboles"/><button onClick={runAI} disabled={aiBusy}>{aiBusy?"Aplicando…":"Aplicar al entorno"}</button><small>Puede añadir objetos, cambiar colores/materiales, mover, girar, escalar, borrar y crear cámaras/luces sobre la escena actual.</small></div><div className="studio-sidebar-spacer"/>
   ...
   <div className="editor-main" onClick={()=>sidebarOpen&&setSidebarOpen(false)}>
    ...
    <div className="editor-control-footer"><button title="Deshacer (Ctrl+Z)" onClick={undo} disabled={!past.length}>↶ Deshacer</button><button title="Rehacer (Ctrl+Y)" onClick={redo} disabled={!future.length}>↷ Rehacer</button><button title={t.selectTool} onClick={()=>setTool("select")}>{t.selectTool}</button><button title={t.move} onClick={()=>setTool("move")}>{t.move}</button><button title={t.rotate} onClick={()=>setTool("rotate")}>{t.rotate}</button><button title={t.scale} onClick={()=>setTool("scale")}>{t.scale}</button><button onClick={dup}>{t.duplicate}</button><button title={t.deleteObject} onClick={del}>{t.deleteObject}</button><button title={t.save} onClick={saveScene}>{t.save}</button></div>
...
function Login(){const[,t]=useLang();const{login}=useUser();const nav=useNavigate();const[name,setName]=useState("");const[email,setEmail]=useState("");const[password,setPassword]=useState("");const[busy,setBusy]=useState(false);return <main className="page narrow auth-page"><div className="form-card auth-card"><div className="eyebrow">{t.account}</div><h1>{t.login}</h1><p className="muted">Cuenta personal para jugar, guardar progreso y ganar F¢.</p><label>{t.email}<input type="email" value={email} onChange={e=>setEmail(e.target.value)}/></label><label>{t.password}<input type="password" value={password} onChange={e=>setPassword(e.target.value)}/></label><label>{t.displayName}<input value={name} onChange={e=>setName(e.target.value)} placeholder="Your name"/></label><button className="button button-primary" disabled={busy||!email||!password} onClick={async()=>{setBusy(true);await login({name:name||"Eskådin Player",email,password});nav("/account")}}>{busy?"…":t.login}</button><Link className="muted" to="/register">{t.register}</Link><Link className="muted" to="/developer/register">{t.createDeveloper} →</Link></div></main>}
function Register(){const[,t]=useLang();const{login}=useUser();const nav=useNavigate();const[name,setName]=useState("");const[email,setEmail]=useState("");const[password,setPassword]=useState("");const[confirm,setConfirm]=useState("");const[skin,setSkin]=useState("#f2c7a5");const[shirt,setShirt]=useState("#5b7cff");const[pants,setPants]=useState("#202638");const[hat,setHat]=useState("none");const preview={name:name||"Eskådin Player",skin,shirt,pants,hat};return <main className="signup-page"><div className="signup-brand">Esk<span>å</span>din St<span>ü</span>dis<sup>®</sup></div><div className="signup-shell"><section className="signup-copy"><span className="roblox-kicker">{t.studyAvatar||t.register}</span><h1>Tu cuenta.<br/><strong>Tu avatar.<br/>Tu mundo.</strong></h1><p>entra a Eskådin Stüdis, crea juegos, descubre experiencias y juega con tu avatar.</p><Avatar user={preview} size="lg"/></section><section className="signup-card"><div className="signup-tabs"><b>CREAR CUENTA</b><Link to="/login">YA TENGO CUENTA</Link></div><h2>{t.register}</h2><label>{t.username}<input value={name} onChange={e=>setName(e.target.value)} placeholder="Tu nombre"/></label><label>{t.email}<input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="tu@email.com"/></label><label>{t.password}<input type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="Contraseña"/></label><label>{t.confirmPassword}<input type="password" value={confirm} onChange={e=>setConfirm(e.target.value)} placeholder="Repite la contraseña"/></label><div className="avatar-custom"><b>{t.customizeAvatar}</b><div><span>{t.skin}</span>{["#f2c7a5","#d89b72","#8d5524","#f7dfc5"].map(c=><button key={c} style={{background:c}} className={skin===c?"chosen":""} onClick={()=>setSkin(c)} aria-label="Color de piel"/>)}</div><div><span>{t.shirt}</span>{["#5b7cff","#e95d6a","#58b89a","#9b6cff","#f2b84b"].map(c=><button key={c} style={{background:c}} className={shirt===c?"chosen":""} onClick={()=>setShirt(c)} aria-label="Color de camiseta"/>)}</div><div><span>{t.pants}</span>{["#202638","#354a73","#4d3430","#29352d"].map(c=><button key={c} style={{background:c}} className={pants===c?"chosen":""} onClick={()=>setPants(c)} aria-label="Color de pantalón"/>)}</div><div><span>{t.accessory}</span>{[["none","Sin accesorio"],["cap","Gorra"],["crown","Corona"]].map(([v,l])=><button key={v} className={"avatar-option "+(hat===v?"chosen":"")} onClick={()=>setHat(v)}>{l}</button>)}</div></div><button className="signup-submit" disabled={!email||!password||password!==confirm} onClick={async()=>{await login({name:name||"Eskådin Player",email,password,skin,shirt,pants,hat});nav("/account")}}>{t.register} <span>→</span></button><small>al continuar aceptas las normas de la comunidad.</small></section></div></main>}
