 ...
 return <main className="page editor-page">
  ...
  <div className={"editor-shell "+(sidebarOpen?"sidebar-open":"")}>
   ...
   <aside className="studio-sidebar" aria-hidden={!sidebarOpen}>
    <div className="studio-sidebar-head"><div className="studio-sidebar-brand">Eskådin<br/><span>Stüdis</span></div><strong className="studio-project-side">{t.newGame}</strong></div><div className="studio-sidebar-actions studio-nav-actions"><Link to="/">⌂ {t.back}</Link><Link to="/games">▶ {t.explore}</Link></div><div className="studio-sidebar-section">{t.language}</div><select className="studio-language" value={lang} onChange={e=>setLang(e.target.value)}>{Object.entries(LANG).map(([k,v])=><option key={k} value={k}>{v}</option>)}</select>
