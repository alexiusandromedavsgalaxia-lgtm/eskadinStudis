...
const EXTRA_T={es:{add:"Añadir",hierarchy:"Jerarquía",assets:"Recursos",scene:"Escena",selectTool:"Seleccionar",move:"Mover",rotate:"Rotar",scale:"Escalar",grid:"Cuadrícula",snap:"Ajuste",wireframe:"Estructura",axes:"Ejes",duplicate:"Duplicar",deleteObject:"Borrar",reset:"Restablecer",fullscreen:"Pantalla completa",inspector:"Inspector",primitives:"PRIMITIVAS",sceneObjects:"OBJETOS DE ESCENA",material:"MATERIAL",quickActions:"ACCIONES RÁPIDAS",resetTransform:"Restablecer transformación",deleteObjectFull:"Borrar objeto"},en:{add:"Add",hierarchy:"Hierarchy",assets:"Assets",scene:"Scene",selectTool:"Select",move:"Move",rotate:"Rotate",scale:"Scale",grid:"Grid",snap:"Snap",wireframe:"Wireframe",axes:"Axes",duplicate:"Duplicate",deleteObject:"Delete",reset:"Reset",fullscreen:"Fullscreen",inspector:"Inspector",primitives:"PRIMITIVES",sceneObjects:"SCENE OBJECTS",material:"MATERIAL",quickActions:"QUICK ACTIONS",resetTransform:"Reset transform",deleteObjectFull:"Delete object"},sv:{add:"Lägg till",hierarchy:"Hierarki",assets:"Resurser",scene:"Scen",selectTool:"Välj",move:"Flytta",rotate:"Rotera",scale:"Skala",grid:"Rutnät",snap:"Fäst",wireframe:"Trådram",axes:"Axlar",duplicate:"Duplicera",deleteObject:"Ta bort",reset:"Återställ",fullscreen:"Helskärm",inspector:"Inspektör",primitives:"PRIMITIVER",sceneObjects:"SCENOBJEKT",material:"MATERIAL",quickActions:"SNABBÅTGÄRDER",resetTransform:"Återställ transform",deleteObjectFull:"Ta bort objekt"},de:{add:"Hinzufügen",hierarchy:"Hierarchie",assets:"Assets",scene:"Szene",selectTool:"Auswählen",move:"Verschieben",rotate:"Drehen",scale:"Skalieren",grid:"Raster",snap:"Einrasten",wireframe:"Drahtmodell",axes:"Achsen",duplicate:"Duplizieren",deleteObject:"Löschen",reset:"Zurücksetzen",fullscreen:"Vollbild",inspector:"Inspektor",primitives:"PRIMITIVEN",sceneObjects:"SZENENOBJEKTE",material:"MATERIAL",quickActions:"SCHNELLAKTIONEN",resetTransform:"Transformation zurücksetzen",deleteObjectFull:"Objekt löschen"},fr:{add:"Ajouter",hierarchy:"Hiérarchie",assets:"Ressources",scene:"Scène",selectTool:"Sélectionner",move:"Déplacer",rotate:"Tourner",scale:"Échelle",grid:"Grille",snap:"Accrochage",wireframe:"Fil de fer",axes:"Axes",duplicate:"Dupliquer",deleteObject:"Supprimer",reset:"Réinitialiser",fullscreen:"Plein écran",inspector:"Inspecteur",primitives:"PRIMITIVES",sceneObjects:"OBJETS DE SCÈNE",material:"MATÉRIAU",quickActions:"ACTIONS RAPIDES",resetTransform:"Réinitialiser la transformation",deleteObjectFull:"Supprimer l’objet"}};
  ...
  <div className="editor-shell">
   <aside className="studio-sidebar">
    ...
    <div className="studio-tool-grid">
     <button className={tool==="select"?"active":""} onClick={()=>setTool("select")}>↖<span>{t.selectTool}</span><kbd>Q</kbd></button>
     <button className={tool==="move"?"active":""} onClick={()=>setTool("move")}>✥<span>{t.move}</span><kbd>W</kbd></button>
    ...
    <div className="studio-sidebar-actions">
     <button onClick={dup}>＋ {t.duplicate}</button><button onClick={del}>⌫ {t.deleteObject}</button><button onClick={reset}>↺ {t.reset}</button>
    </div>
   ...
   <div className="editor-main">
    <div className="editor-workspace">
     ...
     <aside className="editor-dock right-dock">
      ...
      {obj?<><label>Name<input value={obj.name} onChange={e=>upd(obj.id,{name:e.target.value})}/></label><div className="inspector-group"><b>TRANSFORM</b><div className="xyz"><label>X<input type="number" step=".1" value={obj.x} onChange={e=>upd(obj.id,{x:Number(e.target.value)})}/></label><label>Y<input type="number" step=".1" value={obj.y} onChange={e=>upd(obj.id,{y:Number(e.target.value)})}/></label><label>Z<input type="number" step=".1" value={obj.z} onChange={e=>upd(obj.id,{z:Number(e.target.value)})}/></label></div><div className="xyz"><label>RX<input type="number" step="1" value={obj.rx} onChange={e=>upd(obj.id,{rx:Number(e.target.value)})}/></label><label>RY<input type="number" step="1" value={obj.ry} onChange={e=>upd(obj.id,{ry:Number(e.target.value)})}/></label><label>RZ<input type="number" step="1" value={obj.rz} onChange={e=>upd(obj.id,{rz:Number(e.target.value)})}/></label></div><label>{t.scale}<input type="number" step=".1" min=".1" value={obj.s} onChange={e=>upd(obj.id,{s:Number(e.target.value)})}/></label></div><div className="inspector-group"><b>{t.material}</b><label>Color<input type="color" value={obj.color||"#71809a"} onChange={e=>upd(obj.id,{color:e.target.value})}/></label><label>Roughness<input type="range" min="0" max="1" step=".01" value={obj.roughness??.55} onChange={e=>upd(obj.id,{roughness:Number(e.target.value)})}/></label><label>Metalness<input type="range" min="0" max="1" step=".01" value={obj.metalness??.2} onChange={e=>upd(obj.id,{metalness:Number(e.target.value)})}/></label></div><div className="inspector-group"><b>{t.quickActions}</b><div className="inspector-actions"><button onClick={dup}>{t.duplicate}</button><button onClick={()=>upd(obj.id,{x:0,y:.5,z:0,rx:0,ry:0,rz:0,s:1})}>{t.resetTransform}</button><button className="danger-mini" onClick={del}>{t.deleteObjectFull}</button></div></div></>:<div className="empty-inspector">{t.selectObject}</div>}
