...
function useGames(){const{user}=useUser();const cloudKey=user?.cloudKey||null;const[games,setGames]=useState(()=>read("eskadin-games",seed));const syncReady=useRef(false);useEffect(()=>{let cancelled=false;syncReady.current=false;const load=async()=>{try{const res=await fetch("/api/games",{cache:"no-store"});if(!res.ok)throw new Error("cloud get "+res.status);const data=await res.json();const remote=Array.isArray(data.games)?data.games:[];const local=read("eskadin-games",seed);const byId=new Map();[...local,...remote].forEach(g=>{if(g?.id!=null)byId.set(String(g.id),g)});const merged=[...byId.values()];if(cancelled)return;setGames(merged);save("eskadin-games",merged)}catch{if(!cancelled)setGames(read("eskadin-games",seed))}finally{if(!cancelled)syncReady.current=true}};load();return()=>{cancelled=true}},[cloudKey]);useEffect(()=>{save("eskadin-games",games);if(!cloudKey||!syncReady.current)return;const timer=setTimeout(()=>{fetch("/api/games",{method:"PUT",headers:{"Content-Type":"application/json","X-Eskadin-Key":cloudKey},body:JSON.stringify({games})}).catch(()=>{})},350);return()=>clearTimeout(timer)},[games,cloudKey]);return[games,setGames]}
...
function GameRuntime({game,onExit,onRestart}){const[,t]=useLang();
 ...
 useEffect(()=>{
  ...
  renderer.domElement.addEventListener("pointercancel",lookUp);
