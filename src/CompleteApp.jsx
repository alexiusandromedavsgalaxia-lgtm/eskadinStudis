...
function useLang(){const{lang,setLang}=useContext(LangContext);return[lang,{...TXT[lang],...EXTRA_T[lang]},setLang]}
...
function Account(){const[,t]=useLang();const{user,logout}=useUser();const current=user||{name:"Eskådin Player"};return <main className="roblox-page account-page"><div className="profile-hero"><div className="profile-avatar"><Avatar user={current} size="xl"/></div><div><span className="roblox-kicker">PROFILE</span><h1>{current.name}</h1><p>@{normalizeIdentity(current.name)}</p><div className="profile-actions"><Link to="/editor" className="roblox-pill">Crear</Link><Link to="/chat" className="roblox-pill">Chat</Link></div></div></div><div className="profile-grid"><section className="roblox-card"><h2>Sobre mí</h2><p>jugador de Eskådin Stüdis</p><div className="profile-stats"><b>{Number(localStorage.getItem("eskadin-fc")||0)}<small>F¢</small></b><b>0<small>juegos</small></b><b>0<small>sesiones</small></b></div></section><section className="roblox-card"><h2>Avatar</h2><div className="avatar-preview-row"><Avatar user={current} size="md"/><span>tu avatar está listo para jugar.</span></div></section><section className="roblox-card"><h2>Cuenta</h2><div className="account-actions"><Link className="button button-ghost" to="/wallet">{t.wallet}</Link><Link className="button button-ghost" to="/settings">{t.settings}</Link><Link className="button danger-button" to="/account/delete">{t.delete}</Link><button className="button button-ghost" onClick={logout}>{t.logout}</button></div></section></div></main>}
...
function Register(){const[,t]=useLang();const{login}=useUser();const nav=useNavigate();const[name,setName]=useState("");const[skin,setSkin]=useState("#f2c7a5");const[shirt,setShirt]=useState("#5b7cff");const[pants,setPants]=useState("#202638");const[hat,setHat]=useState("none");const preview={name:name||"Eskådin Player",skin,shirt,pants,hat};return <main className="signup-page"><div className="signup-brand">Esk<span>å</span>din St<span>ü</span>dis<sup>®</sup></div><div className="signup-shell"><section className="signup-copy"><span className="roblox-kicker">CREATE YOUR AVATAR</span><h1>Tu cuenta.<br/><strong>Tu avatar.<br/>Tu mundo.</strong></h1><p>entra a Eskådin Stüdis, crea juegos, descubre experiencias y juega con tu avatar.</p><Avatar user={preview} size="lg"/></section><section className="signup-card"><div className="signup-tabs"><b>CREAR CUENTA</b><Link to="/login">YA TENGO CUENTA</Link></div><h2>{t.register}</h2><label>Username<input value={name} onChange={e=>setName(e.target.value)} placeholder="Tu nombre"/></label><label>Email<input type="email" placeholder="tu@email.com"/></label><label>Password<input type="password" placeholder="Contraseña"/></label><label>Confirm password<input type="password" placeholder="Repite la contraseña"/></label><div className="avatar-custom"><b>Personaliza tu avatar</b><div><span>Piel</span>{["#f2c7a5","#d89b72","#8d5524","#f7dfc5"].map(c=><button key={c} style={{background:c}} className={skin===c?"chosen":""} onClick={()=>setSkin(c)} aria-label="Color de piel"/>)}</div><div><span>Camiseta</span>{["#5b7cff","#e95d6a","#58b89a","#9b6cff","#f2b84b"].map(c=><button key={c} style={{background:c}} className={shirt===c?"chosen":""} onClick={()=>setShirt(c)} aria-label="Color de camiseta"/></div><div><span>Pantalón</span>{["#202638","#354a73","#4d3430","#29352d"].map(c=><button key={c} style={{background:c}} className={pants===c?"chosen":""} onClick={()=>setPants(c)} aria-label="Color de pantalón"/></div><div><span>Accesorio</span>{[["none","Sin accesorio"],["cap","Gorra"],["crown","Corona"]].map(([v,l])=><button key={v} className={"avatar-option "+(hat===v?"chosen":"")} onClick={()=>setHat(v)}>{l}</button>)}</div></div><button className="signup-submit" onClick={()=>{login({name:name||"Eskådin Player",skin,shirt,pants,hat});nav("/account")}}>{t.register} <span>→</span></button><small>al continuar aceptas las normas de la comunidad.</small></section></div></main>}
